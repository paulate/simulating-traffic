...
//custom cardinal neighbor code
Grid.getCardinalNeighbor = function(agent,direction){ //direction = north, south, east, west
...
Grid.getNeighbors = function(agent){
	...
	var sightRadius = agent.sightRadius;
	...
	if(hood==Grid.NEIGHBORHOOD_MOORE){
		...
		for (i = -1*sightRadius; i <= sightRadius; i++)
		{
			for (j = -1* sightRadius; j <= sightRadius; j++)
			{
				if (i==0 && j==0)
				{}
				else {coords.push([x + i, y + j])};
