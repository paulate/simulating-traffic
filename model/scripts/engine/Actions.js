...
// SET_SIGHT_RADIUS: UI for setting the sight radius of a specific state
// is there a more efficient way computationally to set a property, rather than update it every tick?
Actions.set_sight_radius = {
	name: "Set sight radius to ... ",

	props: {sightRadius:1},
	
	step: function(agent,config){
		agent.sightRadius = config.sightRadius;
	},

	ui: function(config){

		return EditorHelper()
				.label("Set the sight radius to ")
				.number(config, "sightRadius", {
					integer:true,
					min:0, max:8,
					step:1
				})
...
Actions.move_to = {
	...
	step: function(agent,config){
		...
		}else if(config.space==2){ //north
			spots = Grid.getCardinalNeighbor(agent,"north");
		}else if(config.space==3){ //e
			spots = Grid.getCardinalNeighbor(agent,"east");
		}else if(config.space==4){ //s
			spots = Grid.getCardinalNeighbor(agent,"south");
		}else if(config.space==5){ //w
			spots = Grid.getCardinalNeighbor(agent,"west");
		}
	...
	ui: function(config){
		...
		return EditorHelper()
				...
				.selector([
					...
					{ name:"any", value:1 },
					{ name:"the north", value:2},
					{ name:"the east", value:3},
					{ name:"the south", value:4},
					{ name:"the west", value:5}
